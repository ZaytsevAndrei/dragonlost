...
**Снимок статистики вайпа** — сохраняет текущие `StatisticsDB` всех игроков в `stats_wipe_*` (база для `/stats` «с начала вайпа»). Вызывать вручную, если автоснимок перед вайпом не сработал:

```powershell
npx ts-node src/scripts/snapshotWipeStats.ts
```

**Итоги ТОП фарма** — считает рейтинг (сера ×1 + железо ×0,5 + камень ×0,3 + дерево ×0,05), начисляет призы 500/250/150 ₽, отправляет сообщение в Discord. Обычно запускается cron за ~30 мин до вайпа (17:00 / 20:00 МСК); скрипт — для ручной проверки или повторной отправки:
