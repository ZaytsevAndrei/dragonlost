...
async function playerMeta(steamid: string): Promise<PageMeta> {
  ...
  try {
    ...
    return {
      ...
      description: `Наиграно за вайп: ${hours}; убийств: ${kills}; K/D: ${kd}. Профиль игрока Rust-сервера DragonLost («Волга», x1).`,
      url,
    };
  } catch (error) {
...
router.get('*', async (req: Request, res: Response) => {
  ...
  return sendPage(res, {
    title: 'DragonLost — Rust сервер x1 «Волга» | Вайпы каждые 10 дней',
